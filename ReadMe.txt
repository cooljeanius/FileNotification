...
file notification API. The general recommended usage is to create
a separate thread, open a file descriptor to the file system object,
...
used to pass information into our MP thread, which may not be accessible or
safe to initialize from our thread. We then create the MP thread passing in
the initialized MyMPTaskInfo structure.
...
Either click the "?" from the FileNotification sample application,
or read the enclosed "kqueue.pdf" file for more in-depth information on kqueues.
